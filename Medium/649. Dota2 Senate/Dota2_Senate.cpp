...
public:
	...
	{
		std::queue<char> senate_queue;

		int radiantCount = 0, direCount = 0;

		for (char current_senator : senate) {
			senate_queue.push(current_senator);
			if (current_senator == 'R') { radiantCount++; }
			else { direCount++; }
		}

		int radiantFloatBan = 0, direFloatBan = 0;

		while (radiantCount && direCount) {
			char current = senate_queue.front();
			senate_queue.pop();
...
int main() {
	...
	std::string test_string = "RDD";

	std::cout << test_solution.predictPartyVictory(test_string);
